...
function clickFunc() {
  ...
  function clickNumber(number) {
    ...
    if (number !== "←" || number !== "C") {
      ...
      if (symbol[number]) {
        ...
        if (
          (symbol[number] || symbol[preKey] || resVal == "0") &&
          number !== "."
        ) {
          res.innerHTML = "";
        }
  ...
  about.onclick = history.onclick = function(e) {
    ...
    if (target == "au") {
      ...
      historyBox.children[0].children[0].innerHTML =
        ...
        "<p id='downloadApp'><a href='" +
        appConfig.appUrl +
        "' target='_blank'>点击下载安卓APP</a></p>" +
        "<p><i class='iconfont build'>&#xe60f;</i> Build " +
        appConfig.buildId +
        ". Version：" +
        appConfig.version +
        "</p>" +
  ...
  function saveCalcHistory(val) {
    var key =
      Mybry.wdb.constant.TABLE_NAME +
      Mybry.wdb.constant.SEPARATE +
      Mybry.wdb.getId();
  ...
  function isResOverflow(leng) {
    ...
    var w =
      calc.style.width ||
      getComputedStyle(calc).width ||
      calc.currentStyle.width;
    w = parseInt(w);

    //判断是否是移动端
    if (
      (Mybry.browser.versions.android ||
        Mybry.browser.versions.iPhone ||
        Mybry.browser.versions.iPad) &&
      !symbol[preKey]
    ) {
      if (leng > 15) {
        return true;
  ...
  if (flag) {
    if (styles) {
      ...
      if (w == "80%") {
        calc.setAttribute(
          "style",
          "width: 100%; height: 100%; position: absolute;left: 0px; bottom: 0px;"
        );
      }
    }
    if (posi === "left") {
      ...
      calc.setAttribute(
        "style",
        "width: 80%; height: 70%; position: absolute;right: 0px; bottom: 0px;"
      );
    }
    //计算表达式区域高度
...
function positionFun(postion) {
  if (postion === "left") {
    calc.setAttribute(
      "style",
      "width: 80%; height: 70%; position: absolute;left: 0px; bottom: 0px;"
    );
  } else {
    calc.setAttribute(
      "style",
